...
int CImageClient::DoExecute()
{
	...
	int numRuns = 0;
	int headerLength = 76;
	unsigned char *buf = (unsigned char*) malloc(1500000);
	while (stop == false){
		...
		fprintf(stdout,"%c%c%c%c: %05i - %ix%i - %i\n",pave.signature[0],pave.signature[1],pave.signature[2],pave.signature[3],pave.header_size,pave.encoded_stream_width,pave.encoded_stream_height,pave.frame_type);
